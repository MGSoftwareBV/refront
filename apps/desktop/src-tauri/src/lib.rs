...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![show_window, check_for_updates, send_notification_with_action, navigate_to_path, is_autostart_enabled, set_autostart_enabled, get_pending_notification_navigation])
        .setup(move |app| {
            ...
            let _tray = TrayIconBuilder::new()
                ...
                .on_tray_icon_event(move |tray, event| {
                    match event {
                        // Handle left clicks to open/focus main window (or toggle search if enabled)
                        TrayIconEvent::Click {
                            ...
                            // Check if search window is enabled
                            if let Some(managed_search_state) = app_handle.try_state::<SearchWindowState>() {
                                let search_enabled = *managed_search_state.lock().unwrap();
                                
                                if search_enabled {
                                    // Toggle search window if enabled
                                    let _ = toggle_search_window(app_handle, &managed_search_state);
                                } else {
                                    // Otherwise just open/focus the main window
                                ...
                                // Fallback: just open main window
                                if let Some(main_window) = app_handle.get_webview_window("main") {
                                    let _ = main_window.show();
                                    let _ = main_window.set_focus();
                                }
                        ...
                        // Handle double-click to always open main window
