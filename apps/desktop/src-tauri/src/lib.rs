...
use tauri::{
    Emitter, Listener, Manager, PhysicalPosition, Position, WebviewUrl,
...
// Global state for pending notification navigation (when notification is clicked)
type PendingNotificationNavigation = Arc<Mutex<Option<String>>>;

// Global state for notification badge count
type NotificationBadgeCount = Arc<Mutex<u32>>;
...
/// Send a desktop notification with an optional action URL
/// When clicked, it will show the app and navigate to the path
#[tauri::command]
async fn send_notification_with_action(
    ...
    // Store the action path for when the notification is interacted with
    if let Some(path) = &action_path {
        if let Some(pending_nav) = app.try_state::<PendingNotificationNavigation>() {
            let mut guard = pending_nav.lock().unwrap();
            *guard = Some(path.clone());
            println!("🔔 Stored pending navigation: {}", path);
        }
    }
    ...
    // Use a stable action type id so the frontend can listen for click actions via @tauri-apps/plugin-notification.
    notification = notification.action_type_id("refront-notification");
...
/// Get pending notification navigation path (called by frontend to check for pending navigation)
#[tauri::command]
fn get_pending_notification_navigation(app: tauri::AppHandle) -> Option<String> {
    if let Some(pending_nav) = app.try_state::<PendingNotificationNavigation>() {
        let mut guard = pending_nav.lock().unwrap();
        guard.take()
    } else {
...
/// Navigate to a path in the main window (called from notification click or other sources)
#[tauri::command]
fn navigate_to_path(app: tauri::AppHandle, path: String) -> Result<(), String> {
    if let Some(main_window) = app.get_webview_window("main") {
        ...
        main_window
            .emit("desktop-navigate-to-path", &path)
            .map_err(|e| format!("Failed to emit navigation event: {}", e))?;
    }
    Ok(())
}

/// Update the notification badge count and tray tooltip
#[tauri::command]
fn update_notification_badge(app: tauri::AppHandle, count: u32) -> Result<(), String> {
    // Update the stored count
    if let Some(badge_state) = app.try_state::<NotificationBadgeCount>() {
        let mut guard = badge_state.lock().unwrap();
        *guard = count;
    }
    ...
    if let Some(tray) = app.tray_by_id("main-tray") {
        let tooltip = if count > 0 {
            format!("Refront - {} nieuwe melding{}", count, if count == 1 { "" } else { "en" })
        } else {
            "Refront - Click to open".to_string()
        };
...
#[tauri::command]
fn get_notification_badge(app: tauri::AppHandle) -> u32 {
    if let Some(badge_state) = app.try_state::<NotificationBadgeCount>() {
        *badge_state.lock().unwrap()
    } else {
        0
    }
}
...
/// Silent update check - returns update info without showing dialogs
/// Used for automatic background checks that show a toast in the frontend
#[tauri::command]
async fn check_for_updates_silent(app: tauri::AppHandle) -> Result<Option<String>, String> {
    #[cfg(desktop)]
    {
...
/// Install a pending update - called when user clicks "Update now" in the toast
#[tauri::command]
        ...
        if let Ok(updater) = app.updater() {
            match updater.check().await {
                Ok(Some(update)) => {
                    ...
                    if let Some(main_window) = app.get_webview_window("main") {
                        let _ = main_window.emit("update-installing", true);
                    }
...
async fn create_preloaded_search_window(
    ...
    let search_builder = {
        ...
        .title("Refront Search")
        .inner_size(720.0, 450.0)
        .min_inner_size(720.0, 450.0)
        .resizable(false)
        .user_agent("Mozilla/5.0 (compatible; Refront Desktop App)")
        .transparent(true)
        .decorations(true)
        ...
        // Platform-specific styling (macOS only)
        #[cfg(target_os = "macos")]
        let builder = builder
            .hidden_title(true)
    ...
    let search_window = search_builder.build()?;
...
fn get_app_url() -> String {
    ...
    let env = env::var("REFRONT_ENV")
        .unwrap_or_else(|_| {
            option_env!("REFRONT_ENV")
    ...
    match env.as_str() {
        ...
        "staging" => {
            let url = "https://beta.mgsoftware.nl".to_string();
            println!("🌍 Using staging URL: {}", url);
            url
        },
        "production" | "prod" => {
            let url = "https://tickets.mgsoftware.nl".to_string();
...
fn handle_deep_link_event(app_handle: &tauri::AppHandle, urls: Vec<String>) {
    for url in &urls {
        if url.starts_with("tickets://") {
            // Extract the path from the deep link
            let path = url.strip_prefix("tickets://").unwrap_or("");
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![show_window, check_for_updates, check_for_updates_silent, install_update, send_notification_with_action, navigate_to_path, is_autostart_enabled, set_autostart_enabled, get_pending_notification_navigation, update_notification_badge, get_notification_badge])
        .setup(move |app| {
            ...
            // Create pending notification navigation state
            let pending_nav: PendingNotificationNavigation = Arc::new(Mutex::new(None));
            
            // Create notification badge count state
            let badge_count: NotificationBadgeCount = Arc::new(Mutex::new(0));
            
            // Add states to managed state so commands can access them
            app.manage(search_state.clone());
            app.manage(pending_nav.clone());
            app.manage(badge_count.clone());
            ...
            // Handle window events: close to tray (minimize to tray)
            let window_for_focus = window.clone();
            window.on_window_event(move |event| {
                match event {
                    tauri::WindowEvent::CloseRequested { api, .. } => {
                        // Prevent the window from closing, hide it instead (minimize to tray)
                        api.prevent_close();
            ...
            if !start_minimized {
                ...
                println!("🔧 Started with --minimized; keeping main window hidden");
            }

            // Background mail polling timer - emits event every 30 seconds
            // Frontend listens and checks for new emails
            let window_for_mail = window.clone();
            std::thread::spawn(move || {
                // Wait 10 seconds before starting to allow app to initialize
                std::thread::sleep(std::time::Duration::from_secs(10));
                
                loop {
                    // Emit event to frontend to trigger mail check
                    if let Err(e) = window_for_mail.emit("check-mail-background", ()) {
                        println!("⚠️ Failed to emit check-mail-background event: {}", e);
                    }
            ...
            // Create custom app menu with Help submenu containing "Check for Updates"
            ...
            let app_menu = {
                let app_submenu = Submenu::with_items(
                    ...
                    true,
                    &[
                        &PredefinedMenuItem::about(app_handle, Some("About Refront"), None)?,
                ...
                let check_updates_menu = MenuItem::with_id(app_handle, "menu_check_updates", "Check for Updates...", true, None::<&str>)?;
            ...
            // Handle menu events for "Check for Updates"
            ...
            // Create tray menu with more options
            let open_app_item = MenuItem::with_id(app, "open_app", "Open Refront", true, None::<&str>)?;
            let separator1 = PredefinedMenuItem::separator(app)?;
            let check_updates_item = MenuItem::with_id(app, "check_updates", "Check for Updates...", true, None::<&str>)?;
            let separator2 = PredefinedMenuItem::separator(app)?;
            let quit_item = MenuItem::with_id(app, "quit", "Quit Refront", true, None::<&str>)?;
            ...
            let _tray = TrayIconBuilder::with_id("main-tray")
                ...
                .on_menu_event(|app: &tauri::AppHandle, event| {
                    ...
                    match event.id.as_ref() {
                        ...
                        "quit" => {
                            println!("🔧 Quitting app from tray menu...");
                            app.exit(0);
                        }
                        _ => {}
                    }
                ...
                .on_tray_icon_event(move |tray: &tauri::tray::TrayIcon, event| {
                    match event {
                        // Handle left clicks to always open/focus main window
                        // Search window is only accessible via keyboard shortcut (Shift+Alt+K)
                        TrayIconEvent::Click {
                            ...
                            if let Some(main_window) = app_handle.get_webview_window("main") {
                                ...
                                if let Some(badge_state) = app_handle.try_state::<NotificationBadgeCount>() {
                                    *badge_state.lock().unwrap() = 0;
                                }
                                // Reset tooltip
                                if let Some(tray_icon) = app_handle.tray_by_id("main-tray") {
                                    let _ = tray_icon.set_tooltip(Some("Refront - Click to open"));
                                }
                                // Notify frontend to mark notifications as read
                                let _ = main_window.emit("notifications-cleared", ());
                            }
                        },
                        // Handle double-click to also open main window
