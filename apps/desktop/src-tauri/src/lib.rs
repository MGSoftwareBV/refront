...
/// When clicked on Windows, it will open the app via deep link protocol
#[tauri::command]
async fn send_notification_with_action(
    ...
    // Store the action path for when the app regains focus
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![show_window, check_for_updates, send_notification_with_action, navigate_to_path])
        .setup(move |app| {
            ...
            // Handle window focus events to process pending notification navigation
            let pending_nav_for_focus = pending_nav.clone();
            let window_for_focus = window.clone();
            window.on_window_event(move |event| {
                if let tauri::WindowEvent::Focused(true) = event {
                    // Check if there's a pending navigation from a notification click
                    let mut guard = pending_nav_for_focus.lock().unwrap();
                    if let Some(path) = guard.take() {
                        println!("🔔 Window focused - processing pending navigation: {}", path);
                        // Emit navigation event to frontend
                        let _ = window_for_focus.emit("desktop-navigate-to-path", &path);
                    }
                }
            });
